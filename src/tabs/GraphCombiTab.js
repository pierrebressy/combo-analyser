...
const drawComboProfile = (dataManager) => ({
    id: constants.COMBO_PROFILE_PLUGIN_ID,
    beforeDatasetsDraw(chart) {
        const { ctx, scales, data } = chart;
        const { x, y } = scales;
        const labels = data.labels;
...
export default function GraphCombiTab() {
    ...
    useEffect(() => {
        ...
        const pluginComboProfile = drawComboProfile(dataManager);
        ...
        const existingIndexComboProfile = chart.config.plugins.findIndex(p => p.id === constants.COMBO_PROFILE_PLUGIN_ID);
        if (existingIndexComboProfile !== -1) {
            chart.config.plugins.splice(existingIndexComboProfile, 1);
        }
        chart.config.plugins.push(pluginComboProfile);
    ...
    useEffect(() => {
        ...
        const fetchDataFromDatamanager = async () => {
            try {
                const res = await fetch(constants.TICKERS_LIST);
                const json = await res.json();

                let ticker = dataManager.get_ticker_for_active_combo();
                //setTickers(tickers);
                ...
                console.error("Failed to fetch data from datamanager", err);
            }
        };

        fetchDataFromDatamanager();
