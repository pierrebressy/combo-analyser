...
export function display_p_and_l_graph(p_and_l_area, p_and_l_area_height) {
    ...
    draw_one_sigma_area(svg, get_underlying_current_price(), p_and_l_graph_height);
}
...
export function display_current_price(svg) {
    ...
    const x_position = env.get_x_scale()(get_underlying_current_price());
    let l = new Line(svg, env);
    l.set_position(x_position, -14, x_position, env.get_window_height());
    l.set_color("#0055FF");
    ...
    label.set_width(80);
    label.set_rect_position(
        env.get_window_left_margin() + env.get_x_scale()(get_underlying_current_price()) - label.get_width() / 2,
        env.get_button_default_text_vpos() + 6);
    label.set_text_position(
        env.get_window_left_margin() + env.get_x_scale()(get_underlying_current_price()),
        env.get_button_default_text_vpos() + 8);
    label.set_text(`${get_underlying_current_price().toFixed(1)}`);
    label.set_text_color("white");
    label.text_element.style("cursor", "grabbing")
    label.show();
    ...
    label.text_element
        .call(d3.drag()
            .on("drag", function (event) {
                ...
                set_underlying_current_price(newStrike);
                draw_graph();
            })
        )
        .on("contextmenu", function (event) {
