...
function create_specific_lines_basic() {
    ...
    const black_material = new THREE.LineBasicMaterial({
        color: dark_mode ? 0xffffff : 0x000000 // white in dark mode, black otherwise
    });
...
function create_mesh_monochrome(curve_data) {
    ...
    const materialWireframe = new THREE.LineBasicMaterial({ color: 0x0000FF }); // Black wireframe
...
export function update_3d_view() {
    ...
    // camera = new THREE.PerspectiveCamera(cameraPosition.fov, container.clientWidth / container.clientHeight, 0.1, 1000);
    ...
    camera.position.set(10, 10, 1);  // Camera at (10,10,1)
...
export function update_3d_view_old() {
    ...
    const xx = d3.select(".view3d-graph-container")
    let tab_is_hidden = xx.classed("hidden")
    console.log("tab_is_hidden=", tab_is_hidden);
    ...
    document.getElementById('view3d-graph-container').appendChild(renderer.domElement);
